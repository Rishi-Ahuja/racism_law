          ...
          <div className="flex items-center space-x-1 bg-dark/95 rounded-full p-2 whitespace-nowrap shadow-2xl border border-gold-500/30">
            {navItems.map((item) => (
              <button
                ...
                style={{
                  color: activeSection === item.id ? '#D4AF37' : 'rgba(212, 212, 212, 0.9)',
                }}
              >
        ...
        <div className="w-full bg-dark">
          <div className="flex items-center justify-between px-3 sm:px-4 lg:px-6 py-3 sm:py-4">
            <div className="flex items-center space-x-2 sm:space-x-3 hover-scale inline-flex">
              ...
              <span className="text-base sm:text-lg lg:text-xl font-bold hidden sm:block text-gray-200">
              ...
              {navItems.map((item) => (
                <button
                  ...
                  className={`text-sm font-medium transition-colors duration-200 ${
                    activeSection === item.id ? 'text-gold-500' : 'text-gray-300 hover:text-gold-500'
                  }`}
                >
                  {item.name}
                </button>
            ...
            <button
              ...
              className="lg:hidden p-2 text-gray-300 hover:text-gold-500 transition-colors duration-200"
              ...
              {navItems.map((item) => (
                <button
                  ...
                  className={`block w-full text-left py-2 px-4 rounded-full transition-colors duration-200 font-medium ${
                    activeSection === item.id ? 'bg-gold-500 text-dark' : 'text-gray-300 hover:bg-gold-500/20'
                  }`}
                >
                  {item.name}
                </button>
